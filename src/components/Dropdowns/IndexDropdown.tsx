            ...
            <div className="container mx-auto flex justify-between items-center">
                <nav className="flex space-x-6">
                    {headerNavLinks.map(link => (
                        <div key={link.title} className="flex font-[600] text-[15px] group relative text-[#dbdbdb] hover:text-[#ffffff] active:text-[#ffffff] items-center gap-1 py-5">
                                    ...
                                    <div className="bg-white shadow-[0px_12px_24px_rgba(0,80,125,0.08)] absolute top-[62px] left-1/2 -translate-x-1/2 max-w-[1300px] w-full min-w-fit rounded-xl transition-all overflow-hidden origin-top-left opacity-0 scale-y-0 -translate-y-5 -z-10 flex group-hover:opacity-100 group-hover:scale-y-100 group-hover:translate-y-0 group-hover:z-10">
                                        <div className="p-4 space-y-4">
                                            {link.children.map(child => (
                                                <a key={child.title} className="flex items-center whitespace-nowrap gap-[6px] text-display font-semibold text-[14px] text-black hover:text-[#4ba5dd]" href={child.href}>
                                                    {child.title}
                                                </a>
                                            ))}
                                        </div>
                ...
                <div className="flex items-center justify-between min-w-16 ml-[10px]">
                    <a
                        className="block bg-gradient-to-r from-[#3db1fc] to-[#7f61fe] text-white text-[14px] font-[600] px-5 py-2 rounded-full"
                        href="/dashboard"
                    >
                        Try For Free
                        <Image
